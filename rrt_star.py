...
class RRT():
    ...
    def expand_tree(self, n_steps):
        ...
        while i < limit:            
            ...
            self.closest_tree_points = self.get_closest_points(tree_point, 20)
            id_point_shortest_distance_start = self.get_id_point_shortest_distance_start_via_new_point(tree_point)

            midpoint = self.get_midpoint(tree_point, self.tree_points[id_point_shortest_distance_start][Point.COORDINATES])
            midpoint_1 = self.get_midpoint(midpoint, self.tree_points[id_point_shortest_distance_start][Point.COORDINATES])
            midpoint_2 = self.get_midpoint(midpoint, tree_point)
            if self.check_obstacle_colision(midpoint) or self.check_obstacle_colision(midpoint_1) or self.check_obstacle_colision(midpoint_2):
                continue

            self.tree_points.append([i, tree_point, id_point_shortest_distance_start])
            self.optimalization(i)
            i += 1
    ...
    def get_closest_points(self, point, maximum_distance):
        distance_list = self.calculate_distance_with_other_points(point)
        return [point for point in distance_list if point[Point.DISTANCE] <= maximum_distance]

    def get_id_point_shortest_distance_start_via_new_point(self, tree_point):
        closest_points_to_start = []
        for close_point in self.closest_tree_points:
            distance = self.calculate_distance_to_start(close_point)
            distance += math.dist(tree_point, self.tree_points[close_point[Point.ID]][Point.COORDINATES])
            closest_points_to_start.append([close_point[Point.ID], distance])
        closest_points_to_start.sort(key=self.sorting_distance_function)
        return closest_points_to_start[0][Point.ID]
    ...
    def optimalization(self, i):
        for point in self.closest_tree_points:
            original_distance = self.calculate_distance_to_start(point)
            distance_via_new_point = self.calculate_distance_to_start(self.tree_points[i]) + math.dist(self.tree_points[i][Point.COORDINATES], self.tree_points[point[Point.ID]][Point.COORDINATES])
        
            if original_distance > distance_via_new_point:
                midpoint = self.get_midpoint(self.tree_points[i][Point.COORDINATES], self.tree_points[point[Point.ID]][Point.COORDINATES])
                midpoint_1 = self.get_midpoint(midpoint, self.tree_points[point[Point.ID]][Point.COORDINATES])
                midpoint_2 = self.get_midpoint(midpoint, self.tree_points[i][Point.COORDINATES])
                if self.check_obstacle_colision(midpoint) or self.check_obstacle_colision(midpoint_1) or self.check_obstacle_colision(midpoint_2):
                    continue               
                self.tree_points[point[Point.ID]][Point.NODE_ID] = i
